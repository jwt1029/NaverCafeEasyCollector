...
namespace NaverCafeEditor
{
    public partial class Main : Form, IMyInterface, SettingInterface
    {
        ...
        private int cafeNum = -1;                           //Cafe Club Number
        ...
        private bool schoolNum = false;                     //School Number Exist
        public Main()
        {
        ...
        {
            if (comboBox1.Text == "학번" || comboBox2.Text == "학번" || comboBox3.Text == "학번" || comboBox4.Text == "학번" || comboBox5.Text == "학번" || comboBox6.Text == "학번" || comboBox7.Text == "학번" || comboBox8.Text == "학번")
                schoolNum = true;
            else
                schoolNum = false;
            setNaming();
        ...
        public void ThreadGOGO()
        {
            ...
            while (true)
            {
                ...
                //Check Null Article
                if (isArticleNull(responseFromServer, ArticleNum))
                {
                    ArticleNum--;
                    continue;
                }
        ...
        private bool parser(string src)
        {
            ...
            member[3] = Date.Substring(12, 2);
            member[4] = Date.Substring(15, 2);
        ...
        private bool isArticleNull(string str, int ArticleNum)
        {
            if (str.IndexOf("sType : \"answer\"") != -1)
            {
                string check = str.Substring(str.IndexOf("sType : \"answer\""));
                if (check.IndexOf("sArticleId : " + ArticleNum) != -1)
                    return true;
            }
        ...
        private bool isDateInPeriod(string[] dd)
        {
            ...
            if (dateTimePicker1.Value.Day.ToString().Length == 1)
                ...
                day = dateTimePicker1.Value.Day.ToString();
            if (!(Int64.Parse(dd[0] + dd[1] + dd[2] + dd[3] + dd[4]) >= Int64.Parse(dateTimePicker1.Value.Year.ToString() + dateTimePicker1.Value.Month.ToString() + day + TimeH1.Text + TimeM1.Text)))
            {
                overTime = true;
                return false;
            }
            else
            {
                ...
                if (dateTimePicker2.Value.Day.ToString().Length == 1)
                    ...
                    day = dateTimePicker2.Value.Day.ToString();
                if ((Int64.Parse(dd[0] + dd[1] + dd[2] + dd[3] + dd[4]) <= Int64.Parse(dateTimePicker2.Value.Year.ToString() + dateTimePicker2.Value.Month.ToString() + day + TimeH2.Text + TimeM2.Text)))
                    return true;
            }
            return false;
        }
        ...
        private int getlastArticleNum()
        {
            ...
            responseFromServer = reader.ReadToEnd();
            responseFromServer = responseFromServer.Substring(responseFromServer.IndexOf("<form name=\"ArticleList\" autocomplete=\"off\">") + "<form name=\"ArticleList\" autocomplete=\"off\">".Length);
        ...
        private void FileManager_Click(object sender, EventArgs e)
        {
            ...
            if (cafeNum == -1)
                return;
            TextViewer viewer = new TextViewer(this.textviewerpath, cookie, cafeNum, TargetText.Text);
            viewer.Show();
        }
